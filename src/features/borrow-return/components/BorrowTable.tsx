...
import { Table, TableColumnsType, Avatar, Tag, Select, Button, message } from "antd";
...
const BorrowTable: React.FC = () => {
    ...
    // call api
    const { data: borrowData, isLoading } = useBorrowRecords(currentPage, pageSize, searchValue, filter);
    ...
    const handleNotify = (record: BorrowRecord) => {
        const isOverdue = record.dueDate ? dayjs().isAfter(dayjs(record.dueDate)) : false;
    ...
    const renderStatusTag = (status: string) => {
        const tag = statusMap[status] || { color: "default", text: "Không xác định" };
    ...
    const columns: TableColumnsType<BorrowRecord> = [
        ...
        },
        {
            ...
            render: (_, record: BorrowRecord) => (
                <Link href={`/users/${record.user._id}`} className="flex items-center gap-4">
                    <Avatar
                        size={32}
                    ...
                    >
                        {!record.user.avatar ? record.user.fullName.charAt(0).toUpperCase() : null}
                    </Avatar>
...

        {
            ...
            key: 'book',
            render: (_, record: BorrowRecord) => (
                <Link href={`/books/${record.book._id}`} className="flex items-center gap-4">
                    <Avatar
                        shape="square"
                        size={32}
                        src={record.book.coverImage || null}
                        style={{
                            backgroundColor: !record.book.coverImage ? '#87d068' : 'transparent',
                            color: !record.book.coverImage ? '#fff' : undefined,
                        }}
                    >
                        {!record.book.coverImage ? record.book.title.charAt(0).toUpperCase() : null}
                    </Avatar>
    ...
    return (
        <>
            ...
            <ReturnBookModal openModal={openReturnModal} setOpenModal={setOpenReturnModal} recordId={selectedData?._id} />
        </>
    );
};
